...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.badarshahzad.android.earthreport "

INSTRUMENTATION_OPTS=" \
--skip-class=com.badarshahzad.android.earthreport.R \
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.badarshahzad.android.earthreport$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.badarshahzad.android.earthreport.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall com.badarshahzad.android.earthreport > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall com.badarshahzad.android.earthreport.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear com.badarshahzad.android.earthreport > /dev/null; \
fi)
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.badarshahzad.android.earthreport.test/android.support.test.runner.AndroidJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces com.badarshahzad.android.earthreport ./traces/$(TEST_NAME) force-clean
...
.carved-all-selected-all : $(ESPRESSO_TESTS)
	@echo "$(SELECT_ALL_CARVING_OPTIONS)"
	@export ABC_CONFIG=$(ABC_CFG) && \
